...
interpretAction (pos@(x, y), stack, angle, angleOffset) (Draw Backward) rest =
  let newPos = (x - cos angle, y - sin angle)
   in Line [pos, newPos] : interpret (newPos, stack, angle, angleOffset) rest
interpretAction ((x, y), stack, angle, angleOffset) (Move Forward) rest =
  let newPos = (x + cos angle, y + sin angle)
   in interpret (newPos, stack, angle, angleOffset) rest
interpretAction ((x, y), stack, angle, angleOffset) (Move Backward) rest =
  let newPos = (x - cos angle, y - sin angle)
   in interpret (newPos, stack, angle, angleOffset) rest
interpretAction (pos, stack, angle, angleOffset) (Rotation RightSide) rest =
  interpret (pos, stack, angle + angleOffset, angleOffset) rest
interpretAction (pos, stack, angle, angleOffset) (Rotation LeftSide) rest =
  interpret (pos, stack, angle - angleOffset, angleOffset) rest
interpretAction (pos, stack, angle, angleOffset) Push rest =
  interpret (pos, (pos, angle) : stack, angle, angleOffset) rest
interpretAction (_, (pos, angle) : stack, _, angleOffset) Pop rest =
  interpret (pos, stack, angle, angleOffset) rest
